...
// @grant           GM_addElement
// @require      https://kid4rm90s.github.io/WazeWrap/WazeWrap.js
// @require         https://raw.githubusercontent.com/JS55CT/WazeWrap/refs/heads/master/WazeWrap.js
...
https: (function () {
  ...
  // Custom Alert System (fallback for WazeWrap.Alerts)
  const CustomAlert = {
    container: null,
    alertCounter: 0,

    // Initialize the alert container and inject CSS
    init() {
      if (this.container) return;

      // Inject CSS for custom alerts
      const alertCSS = `
        #wme-poi-shortcuts-alert-container {
          position: fixed;
          top: 60px;
          right: 20px;
          z-index: 10000;
        ...
        .wme-poi-alert.alert-info {
          border-left-color: #17a2b8;
        }

        .wme-poi-alert.alert-success {
          border-left-color: #28a745;
        }

        .wme-poi-alert.alert-warning {
          border-left-color: #ffc107;
          background: #fffbf0;
        }

        .wme-poi-alert.alert-error {
        ...
        .wme-poi-alert-close:hover {
          color: #333;
        }

        .wme-poi-alert.fade-out {
      ...
      injectCSSWithID('wme-poi-shortcuts-alert-styles', alertCSS);

      // Create container
      this.container = document.createElement('div');
      this.container.id = 'wme-poi-shortcuts-alert-container';
    ...
    show(title, message, type = 'info', duration = 3000) {
      ...
      const alertId = `wme-poi-alert-${this.alertCounter++}`;
      const alertEl = document.createElement('div');
      alertEl.className = `wme-poi-alert alert-${type}`;
      alertEl.id = alertId;
      alertEl.style.position = 'relative';
      alertEl.style.paddingRight = '30px';

      alertEl.innerHTML = `
        <button class="wme-poi-alert-close" onclick="this.parentElement.remove()">&times;</button>
        <div class="wme-poi-alert-title">${title}</div>
        <div class="wme-poi-alert-message">${message}</div>
      `;
  ...
  // Smart Alert Wrapper - uses WazeWrap if available, otherwise uses CustomAlert
  const SmartAlert = {
    info(title, message, buttonText = false, useHTMLString = false, duration = 3000) {
      try {
        if (typeof WazeWrap !== 'undefined' && WazeWrap.Alerts) {
          WazeWrap.Alerts.info(title, message, buttonText, useHTMLString, duration);
        } else {
          CustomAlert.info(title, message, duration);
        }
      } catch (e) {
        Logger.warn('WazeWrap.Alerts failed, using fallback:', e);
        CustomAlert.info(title, message, duration);
      }
    },

    success(title, message, buttonText = false, useHTMLString = false, duration = 3000) {
      try {
        if (typeof WazeWrap !== 'undefined' && WazeWrap.Alerts) {
          WazeWrap.Alerts.success(title, message, buttonText, useHTMLString, duration);
        } else {
          CustomAlert.success(title, message, duration);
        }
      } catch (e) {
        Logger.warn('WazeWrap.Alerts failed, using fallback:', e);
        CustomAlert.success(title, message, duration);
    ...
    warning(title, message, buttonText = false, useHTMLString = false, duration = 3000) {
      try {
        if (typeof WazeWrap !== 'undefined' && WazeWrap.Alerts) {
          WazeWrap.Alerts.warning(title, message, buttonText, useHTMLString, duration);
        } else {
          CustomAlert.warning(title, message, duration);
        }
      } catch (e) {
        Logger.warn('WazeWrap.Alerts failed, using fallback:', e);
        CustomAlert.warning(title, message, duration);
    ...
    error(title, message, buttonText = false, useHTMLString = false, duration = 3000) {
      try {
        if (typeof WazeWrap !== 'undefined' && WazeWrap.Alerts) {
          WazeWrap.Alerts.error(title, message, buttonText, useHTMLString, duration);
        } else {
          CustomAlert.error(title, message, duration);
        }
      } catch (e) {
        Logger.warn('WazeWrap.Alerts failed, using fallback:', e);
        CustomAlert.error(title, message, duration);
  ...
  */
      ...
      },
      {
        ...
        func: function (ev) {
          ensureHazardLayersEnabled('layer-switcher-item_permanent_hazard_toll_booth', () => {
            SmartAlert.info('POI Shortcut', `POI Type: <b>Toll Booth</b>`, false, true, 2000);
            $("wz-icon[name='toll-booth']").parent().trigger('click');
          });
        },
      ...
      },
      {
        ...
        func: function (ev) {
          ensureHazardLayersEnabled('layer-switcher-item_permanent_hazard_railroad_crossing', () => {
            SmartAlert.info('POI Shortcut', `POI Type: <b>Level Crossing</b>`, false, true, 2000);
            $("wz-icon[name='railway-crossing']").parent().trigger('click');
          });
        },
        key: -1, // No default key, user can set custom
      ...
      },
      {
        ...
        func: function (ev) {
          ensureHazardLayersEnabled('layer-switcher-item_permanent_hazard_school_zone', () => {
            SmartAlert.info('POI Shortcut', `POI Type: <b>School Zone</b>`, false, true, 2000);
            $("wz-icon[name='school-zone']").parent().trigger('click');
          });
        },
        key: -1, // No default key, user can set custom
      ...
      },
      {
        ...
        func: function (ev) {
          ensureHazardLayersEnabled('layer-switcher-item_permanent_hazard_dangerous_curve', () => {
            SmartAlert.info('POI Shortcut', `POI Type: <b>Sharp Curves</b>`, false, true, 2000);
      ...
      },
      {
        ...
        func: function (ev) {
          ensureHazardLayersEnabled('layer-switcher-item_permanent_hazard_dangerous_intersection', () => {
            SmartAlert.info('POI Shortcut', `POI Type: <b>Complex Junctions</b>`, false, true, 2000);
      ...
      },
      {
        ...
        func: function (ev) {
          ensureHazardLayersEnabled('layer-switcher-item_permanent_hazard_dangerous_merge', () => {
            SmartAlert.info('POI Shortcut', `POI Type: <b>Multiple Lanes Merging</b>`, false, true, 2000);
  ...
  function convertOtherToResidential(wmeSDK) {
    try {
      ...
      SmartAlert.error('POI Shortcut', 'An unexpected error occurred during conversion.', false, false, 3000);
    }
  }
  ...
  function getSelectedVenue(wmeSDK) {
    ...
    if (!selection || selection.objectType !== 'venue' || !selection.ids || selection.ids.length !== 1) {
      SmartAlert.warning('POI Shortcut', 'Please select a venue first.', false, false, 3000);
      return null;
    }
    ...
    if (!venue) {
      SmartAlert.error('POI Shortcut', 'Venue not found.', false, false, 3000);
      return null;
    }
  ...
  function isValidOtherVenue(venue) {
    ...
    if (!isOther) {
      SmartAlert.warning('POI Shortcut', `This function only works with venues of type OTHER. Actual: ${venueCategories.join(', ')}`, false, false, 3000);
      return false;
    }
  ...
  function hasValidPrimaryName(venue) {
    if (!venue.name || !venue.name.trim()) {
      SmartAlert.warning('POI Shortcut', 'Primary name is empty and cannot be used as house number.', false, false, 3000);
      return false;
    }
  ...
  function checkAndUpdateVenueAddress(wmeSDK, venue) {
    checkExistingHouseNumber(wmeSDK, venue)
      .then((hasHouseNumber) => {
        if (hasHouseNumber) {
          SmartAlert.warning('POI Shortcut', 'Venue already has a house number in its address.', false, false, 3000);
          return;
        }
      ...
      .catch((error) => {
        ...
        SmartAlert.error('POI Shortcut', 'Failed to update venue address.', false, false, 3000);
      });
  }
  ...
  function triggerResidentialConversion(venueName) {
    ...
    setTimeout(() => {
      ...
      if (buttonClicked) {
        SmartAlert.info('POI Shortcut', `Successfully converted venue "${venueName}" to residential.`, false, false, 1000);
      } else {
        SmartAlert.warning('POI Shortcut', `House number set to "${venueName}". Please manually click "Convert to residential" button.`, false, false, 3000);
      }
    }, 500);
  }
  ...
  function createPOIFromShortcut(slotNumber, wmeSDK) {
    try {
      ...
      SmartAlert.info('POI Shortcut', `Selected POI Name: <b>${poiName}</b><br>Lock Level: <b>${lockLevel}</b><br>Type: <b>${areaType}</b>`, false, true, 2500);
  ...
  function WMEKSSaveKeyboardShortcuts(scriptName) {
    ...
    SmartAlert.success('POI Shortcut', `Saving keyboard shortcuts for ${scriptName}`, false, false, 3000);
    const shortcuts = [];
  ...
  function swapPrimaryAndAliasNames(wmeSDK, aliasIndex = 0) {
    ...
    try {
      ...
      SmartAlert.info('POI Shortcut', `Swapped names: "<b>${currentPrimaryName}</b>" ↔ "<b>${targetAlias}</b>"`, false, true, 3000);
        ...
        if (lockRank !== undefined && lockRank !== null && lockRank !== venue.lockRank) {
          setTimeout(() => {
            try {
              ...
              SmartAlert.info('NOC Update', `<b>Case 1:</b> Empty gas station updated<br>Primary: <b>NOC</b><br>Brand: <b>Nepal Oil Corporation</b><br>Lock Rank: <b>${venue.lockRank + 1}</b> → <b>${lockRank + 1}</b>`, false, false, 3000)
            } catch (err) {
              Logger.warn('[NOC Debug] lockRank update failed:', err);
              SmartAlert.warning('NOC Update', `<b>Case 1:</b> Empty gas station updated<br>Primary: <b>NOC</b><br>Brand: <b>Nepal Oil Corporation</b><br>⚠️ Lock rank update failed`, false, false, 3000)
            }
          }, RETRY_INJECTION_DELAY * 3);
        } else {
          const lockMessage = lockRank !== undefined && lockRank !== null ? `<br>Lock Rank: <b>${lockRank + 1}</b> (unchanged)` : '';
          SmartAlert.info('NOC Update', `<b>Case 1:</b> Empty gas station updated<br>Primary: <b>NOC</b><br>Brand: <b>Nepal Oil Corporation</b>${lockMessage}`, false, false, 3000)
        }
      } catch (err) {
        Logger.error('NOC: Error updating venue (Case 1):', err);
        SmartAlert.error('NOC Error', 'Failed to update empty gas station', false, false, 3000)
        return;
      }
      return;
    }
        ...
        if (lockRank !== undefined && lockRank !== null && lockRank !== venue.lockRank) {
          setTimeout(() => {
            try {
              wmeSDK.DataModel.Venues.updateVenue({ venueId: venueId, lockRank: lockRank });
              Logger.info('[NOC Debug] lockRank updated successfully:', lockRank);
              SmartAlert.info('NOC Update', `<b>Case 2:</b> Smart swap completed<br>Primary: <b>${newPrimaryName}</b><br>NOC moved to aliases<br>Lock Rank: <b>${venue.lockRank + 1}</b> → <b>${lockRank + 1}</b>`, false, false, 3000)
            } catch (err) {
              Logger.warn('[NOC Debug] lockRank update failed:', err);
              SmartAlert.warning('NOC Update', `<b>Case 2:</b> Smart swap completed<br>Primary: <b>${newPrimaryName}</b><br>NOC moved to aliases<br>⚠️ Lock rank update failed`, false, false, 3000)
            }
          }, RETRY_INJECTION_DELAY * 3);
        } else {
          const lockMessage = lockRank !== undefined && lockRank !== null ? `<br>Lock Rank: <b>${lockRank + 1}</b> (unchanged)` : '';
          SmartAlert.info('NOC Update', `<b>Case 2:</b> Smart swap completed<br>Primary: <b>${newPrimaryName}</b><br>NOC moved to aliases${lockMessage}`, false, false, 3000)
        }
      } catch (err) {
        Logger.error('NOC: Error updating venue (Case 2):', err);
        SmartAlert.error('NOC Error', 'Failed to swap NOC with alias name', false, false, 3000)
        return;
      }
      return;
    }
        ...
        if (lockRank !== undefined && lockRank !== null && lockRank !== venue.lockRank) {
          setTimeout(() => {
            try {
              wmeSDK.DataModel.Venues.updateVenue({ venueId: venueId, lockRank: lockRank });
              Logger.info('[NOC Debug] lockRank updated successfully:', lockRank);
              SmartAlert.info('NOC Update', `<b>Case 3:</b> NOC added as alias<br>Primary: <b>${currentName}</b><br>NOC added to aliases<br>Lock Rank: <b>${venue.lockRank + 1}</b> → <b>${lockRank + 1}</b>`, false, false, 3000)
            } catch (err) {
              Logger.warn('[NOC Debug] lockRank update failed:', err);
              SmartAlert.warning('NOC Update', `<b>Case 3:</b> NOC added as alias<br>Primary: <b>${currentName}</b><br>NOC added to aliases<br>⚠️ Lock rank update failed`, false, false, 3000)
            }
          }, RETRY_INJECTION_DELAY * 3);
        } else {
          const lockMessage = lockRank !== undefined && lockRank !== null ? `<br>Lock Rank: <b>${lockRank + 1}</b> (unchanged)` : '';
          SmartAlert.info('NOC Update', `<b>Case 3:</b> NOC added as alias<br>Primary: <b>${currentName}</b><br>NOC added to aliases${lockMessage}`, false, false, 3000)
        }
      } catch (err) {
        Logger.error('NOC: Error updating venue (Case 3):', err);
        SmartAlert.error('NOC Error', 'Failed to add NOC as alias', false, false, 3000)
        return;
      }
      return;
    }
      ...
      if (lockRank !== undefined && lockRank !== null && lockRank !== venue.lockRank) {
        setTimeout(() => {
          try {
            ...
            SmartAlert.info('NOC Update', `<b>Edge Case:</b> NOC already in aliases<br>Lock rank updated: <b>${venue.lockRank + 1}</b> → <b>${lockRank + 1}</b>`, false, false, 3000)
          } catch (err) {
            Logger.warn('[NOC Debug] lockRank update failed:', err);
            SmartAlert.warning('NOC Update', `<b>Edge Case:</b> NOC already in aliases<br>⚠️ Lock rank update failed`, false, false, 3000)
          }
        }, RETRY_INJECTION_DELAY * 3);
      } else {
        Logger.info('NOC: No changes needed - NOC exists and lock rank unchanged');
        SmartAlert.info('NOC Update', '<b>No Changes:</b> NOC already in aliases<br>Lock rank already correct', false, false, 2500)
      }
      return;
    }
      ...
      if (lockRank !== undefined && lockRank !== null && lockRank !== venue.lockRank) {
        setTimeout(() => {
          try {
            wmeSDK.DataModel.Venues.updateVenue({ venueId: venueId, lockRank: lockRank });
            Logger.info(`[NOC Debug] lockRank updated successfully: ${lockRank} (was ${venue.lockRank})`);
            SmartAlert.info('NOC Update', `<b>Edge Case:</b> NOC already primary<br>Lock rank updated: <b>${venue.lockRank + 1}</b> → <b>${lockRank + 1}</b>`, false, false, 3000)
          } catch (err) {
            Logger.warn('[NOC Debug] lockRank update failed:', err);
            SmartAlert.warning('NOC Update', `<b>Edge Case:</b> NOC already primary<br>⚠️ Lock rank update failed`, false, false, 3000)
          }
        }, RETRY_INJECTION_DELAY * 3);
      } else {
        Logger.info('NOC: No changes needed - NOC is primary and lock rank unchanged');
        SmartAlert.info('NOC Update', '<b>No Changes:</b> NOC already primary<br>Lock rank already correct', false, false, 2500)
      }
      return;
    }
  }
      ...
      if (lockRank !== undefined && lockRank !== null && lockRank !== venue.lockRank) {
        setTimeout(() => {
          try {
            ...
            SmartAlert.warning('Charging Station Updated', `<b>Network:</b> ${primaryName}<br><b>Brand:</b> ${brand}<br><b>Aliases:</b> ${aliases.length > 0 ? aliases.join(', ') : 'None'}<br>⚠️ Lock rank update failed`, false, true, 3000);
          }
        }, RETRY_INJECTION_DELAY * 3);
      } else {
        ...
        SmartAlert.success('Charging Station Updated', `<b>Network:</b> ${primaryName}<br><b>Brand:</b> ${brand}<br><b>Aliases:</b> ${aliases.length > 0 ? aliases.join(', ') : 'None'}${lockMessage}${additionalInfo}`, false, true, 4000);
      }
    } catch (error) {
      Logger.error('[Charging Station] Error updating:', error);
      SmartAlert.error('Charging Station Error', `Failed to update to ${primaryName}`, false, false, 3000)
    }
  }
      ...
      if (lockRank !== undefined && lockRank !== null && lockRank !== venue.lockRank) {
        setTimeout(() => {
          try {
            ...
            SmartAlert.warning('Gas Station Updated', `<b>Brand:</b> ${primaryName}<br><b>Company:</b> ${brand}<br><b>Aliases:</b> ${aliases.length > 0 ? aliases.join(', ') : 'None'}<br>⚠️ Lock rank update failed`, false, true, 3000);
          }
        }, RETRY_INJECTION_DELAY * 3);
      } else {
        const lockMessage = lockRank !== undefined && lockRank !== null ? `<br><b>Lock Rank:</b> ${lockRank + 1} (unchanged)` : '';
        SmartAlert.success('Gas Station Updated', `<b>Brand:</b> ${primaryName}<br><b>Company:</b> ${brand}<br><b>Aliases:</b> ${aliases.length > 0 ? aliases.join(', ') : 'None'}${lockMessage}`, false, true, 3000);
      }
    } catch (error) {
      Logger.error('[Gas Station] Error updating:', error);
      SmartAlert.error('Gas Station Error', `Failed to update to ${primaryName}`, false, false, 3000);
    }
  }
